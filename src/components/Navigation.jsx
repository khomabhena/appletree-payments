...
const Navigation = () => {
  ...
  return (
    <nav className=' fixed top-0 left-0 right-0 bg-black flex justify-between items-center px-4 h-18'>
      ...
      <ul className=' hidden lg:flex gap-8 text-lg pr-4' style={{ color: textColor }}>
      ...
      {!mobileNav && <img onClick={() => {setMobileNav(!mobileNav)}} className=' lg:hidden cursor-pointer h-8 w-8' src='/menu-icon-white.svg' width={'auto'} height={'24px'}  />}
      {mobileNav && <img onClick={() => {setMobileNav(!mobileNav)}} className=' lg:hidden cursor-pointer h-10 w-10' src='/close-icon-white.svg' width={'auto'} height={'32px'}  />}
    </nav>
  )
}
